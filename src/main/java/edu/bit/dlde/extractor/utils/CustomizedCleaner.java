package edu.bit.dlde.extractor.utils;
...
public class CustomizedCleaner {
	...
	final List<String> _tagToDel = Arrays.asList("SCRIPT", "STYLE",
			"INPUT", "BUTTON");
	 ...
	 *            文件路径
	 */
	public void formatHtml(String file) {
		HtmlCleaner cleaner = new HtmlCleaner();
		CleanerProperties props = cleaner.getProperties();
		props.setNamespacesAware(false);
		_list = new ArrayList<String>();
...
//		props.setOmitDeprecatedTags(true);
		...
		JDomSerializer s = null;
		try {
			node = (TagNode) cleaner.clean(reader)
					.getElementListByName("body", false).get(0);
